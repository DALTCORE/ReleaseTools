...
class Status extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        CLI::output($output, 'ReleaseTools filesystem check: ' . ($this->readyState == true ? 'Valid' : 'Invalid'), CLI::INFO, 0);

        $finder->files()->in(Constants::unreleased_dir());
        CLI::output($output, 'New changelog entries to parse: ' .$finder->count(), CLI::INFO);
    }
}
